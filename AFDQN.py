...
Transition = namedtuple('Transition', ['state', 'action', 'reward', 'next_state', 'next_legal_actions', 'done', 'legal_actions'])


class AFDQNAgent(object):
    '''
    ...
    def __init__(self,
        ...
        self.q_estimator = Estimator(num_actions=num_actions, learning_rate=learning_rate, state_shape=state_shape, \
                                     mlp_layers=mlp_layers, device=self.device, lamb=lamb)
    ...
    def predict(self, state):
        ...
        q_values, e_values = self.q_estimator.predict_nograd(np.expand_dims(state['obs'], 0))
        q_values = q_values[0]
    ...
    def train(self):
        ...
        '''
        state_batch, action_batch, reward_batch, next_state_batch, next_legal_actions_batch,\
        done_batch, legal_actions_batch = self.memory.sample()
        # Calculate best next actions using Q-network (Double DQN)
        q_values_next, e_values_next = self.q_estimator.predict_nograd(next_state_batch)
        legal_actions = []
        for b in range(self.batch_size):
            legal_actions.extend([i + b * self.num_actions for i in next_legal_actions_batch[b]])
        ...
        q_values_next_target, e_values_next_target = self.target_estimator.predict_nograd(next_state_batch)
        target_batch = reward_batch + np.invert(done_batch).astype(np.float32) * \
                       self.discount_factor * q_values_next_target[np.arange(self.batch_size), best_actions]

        # Perform gradient descent update
        state_batch = np.array(state_batch)
        loss = self.q_estimator.update(state_batch, action_batch, target_batch, legal_actions_batch)
    ...
    def feed_memory(self, state, action, reward, next_state,
                    next_legal_actions, done, legal_actions):
        ...
        self.memory.save(state, action, reward, next_state, next_legal_actions, done, a)
...
class Estimator(object):
    ...
    def predict_nograd(self, s):
        ...
        with torch.no_grad():
            ...
            q_as, e_as = self.qnet(s)
        return q_as.cpu().numpy(), e_as.cpu().numpy()

    def update(self, s, a, y, e):
        ...
        # update model
        policy_loss = self.mse_loss(Q, y)
        action_loss = self.bce_loss(e_as, e)
        # action_loss = self.mse_loss(e_as, e)

        batch_loss = policy_loss + action_loss * self.lamb # 改变这个系数如（0.1,0.5,1.0,1.5,2.0)
...
class EstimatorNetwork(nn.Module):
    ...
    def __init__(self, num_actions=2, state_shape=None, mlp_layers=None):
        ...
        self.q_net = nn.Sequential(
            nn.Linear(512, self.num_actions, bias=True)
        )
    ...
    def forward(self, s):
        ...
        '''
        feat = self.feature_net(s)
        out = self.q_net(feat)
        e_out = self.e_net(feat)
        return out, e_out
...
class Memory(object):
    ...
    def save(self, state, action, reward, next_state, next_legal_actions, done, legal_actions):
        ...
        if len(self.memory) == self.memory_size:
            self.memory.pop(0)
        transition = Transition(state, action, reward, next_state, next_legal_actions, done, legal_actions)
