...
#     cfr_agent,
# ])
pt_path = 'res/baseline0'
model = torch.load(os.path.join(pt_path, 'model.pth'))
our_agent = DQNAgent(num_actions=env.num_actions,
                     state_shape=env.state_shape[0],
                     mlp_layers=[512, 512],
                     )
our_agent.q_estimator.qnet.load_state_dict(model.q_estimator.qnet.state_dict())
env.set_agents([
    our_agent,
    cfr_agent
])
